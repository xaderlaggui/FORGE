...
export function ForgeButton({
  ...
  const styles = useStyles(T) as any;
  ...
  React.useEffect(() => {
    if (pulse && !disabled && !loading) {
      scale.value = withRepeat(
        withTiming(1.04, { duration: T.motion.duration.pulse }),
        -1,
        true
      );
    } else {
      scale.value = withTiming(1, { duration: T.motion.duration.fast });
    }
    ...
    <Animated.View style={[pulseStyle, style]}>
      ...
      >
        {loading ? (
          <ActivityIndicator
            size="small"
            color={variant === 'primary' ? '#000' : variant === 'danger' ? '#fff' : T.colors.forge}
          />
        ) : (
          <View style={styles.inner}>
            ...
            <Text
              style={[
                styles.label,
                (styles as any)[`label_${size}`],
                (styles as any)[`labelColor_${variant}`],
